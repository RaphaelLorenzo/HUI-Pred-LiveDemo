...
import einops
try:
    from predictors.MotionBERT.lib.model.drop import DropPath
except ImportError:
    # if not running from the root directory
...
class Attention(nn.Module):
    ...
    def forward(self, x, seqlen=1):
        ...
        if self.mode == 'series':
            ...
            qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4) # [3, B*T, Heads, J, dim_feat//Heads]
            q, k, v = qkv[0], qkv[1], qkv[2]   # make torchscript happy (cannot use tensor as tuple)
            x = self.forward_temporal(q, k, v, seqlen=seqlen)
    ...
    def forward_spatial(self, q, k, v):
        B, _, N, C = q.shape # [B*T, Heads, J, dim_feat//Heads]
        attn = (q @ k.transpose(-2, -1)) * self.scale # [B*T, Heads, J, J]
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        x = attn @ v # [B*T, Heads, J, dim_feat//Heads]
        x = x.transpose(1,2).reshape(B, N, C*self.num_heads) # [B*T, J, dim_feat]
        return x
        
    def forward_temporal(self, q, k, v, seqlen=8):
        B, _, N, C = q.shape # [B*T, Heads, J, dim_feat//Heads]
        qt = q.reshape(-1, seqlen, self.num_heads, N, C).permute(0, 2, 3, 1, 4) #(B, H, N, T, C) ie (B, Heads, J, T, dim_feat//Heads)
        kt = k.reshape(-1, seqlen, self.num_heads, N, C).permute(0, 2, 3, 1, 4) #(B, H, N, T, C) ie (B, Heads, J, T, dim_feat//Heads)
        vt = v.reshape(-1, seqlen, self.num_heads, N, C).permute(0, 2, 3, 1, 4) #(B, H, N, T, C) ie (B, Heads, J, T, dim_feat//Heads)

        attn = (qt @ kt.transpose(-2, -1)) * self.scale # (B, Heads, J, T, T)

        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        x = attn @ vt #(B, H, N, T, C) ie (B, Heads, J, T, dim_feat//Heads)
        x = x.permute(0, 3, 2, 1, 4) # (B, T, J, Heads, dim_feat//Heads)
        x = x.reshape(B, N, C*self.num_heads) # (B*T, J, dim_feat)
        return x
...
class DSTformer(nn.Module):
    def __init__(self, 
                 ...
                 desired_return="final", # "final", "intermediates", "embeddings", "representation"
                 att_fuse=True):
    ...
    def forward(self, x):
        ret = None 
        B, T, J, C = x.shape                                    # [B, T, J, C]
        x = x.reshape(-1, J, C)                                 # [B*T, J, C]
        BT = x.shape[0]
        x = self.joints_embed(x)                                # [B*T, J, dim_feat]        
        x = x + self.pos_embed                                  # learnable joints embeddings
        _, J, C = x.shape
        x = x.reshape(-1, T, J, C) + self.temp_embed[:,:T,:,:]  # [B, T, J, dim_feat] + learnable temporal embeddings
        x = x.reshape(BT, J, C)                                 # [B*T, J, dim_feat]
        x = self.pos_drop(x)                                    # dropout of positional embeddings
        
        intermediates = []
        for idx, (blk_st, blk_ts) in enumerate(zip(self.blocks_st, self.blocks_ts)):
            x_st = blk_st(x, T)                                 # [B*T, J, dim_feat]
            x_ts = blk_ts(x, T)                                 # [B*T, J, dim_feat]
            if self.att_fuse:
                att = self.ts_attn[idx]
                alpha = torch.cat([x_st, x_ts], dim=-1)         # [B*T, J, dim_feat*2]
                BT, J = alpha.shape[:2]
                alpha = att(alpha)                                  # [B*T, J, 2] i.e. estimate the weight of the spatial-temporal and temporal-spatial features per joint
                alpha = alpha.softmax(dim=-1)                       # [B*T, J, 2] i.e. estimate the weight of the spatial-temporal and temporal-spatial features per joint
                x = x_st * alpha[:,:,0:1] + x_ts * alpha[:,:,1:2]   # [B*T, J, dim_feat] i.e. weighted sum of the spatial-temporal and temporal-spatial features per joint
            else:
                x = (x_st + x_ts)*0.5
            intermediates.append(x.reshape(B, T, J, -1))
            
        intermediates = torch.stack(intermediates, dim=0)       # [depth, B, T, J, dim_feat]
        if self.desired_return == "intermediates":
            intermediates = einops.rearrange(intermediates, 'L B T J D -> B T J (D L)') # move the depth dimension to the last dimension and flatten the depth dimension into channels
            ret = intermediates.clone()
        x = self.norm(x)                                        # [B*T, J, dim_feat]
        x = x.reshape(B, T, J, -1)                              # [B, T, J, dim_feat]
        if self.desired_return == "embeddings":
            ret = x.clone()
        x = self.pre_logits(x)                                  # [B, T, J, dim_rep]
        if self.desired_return == "representation":
            ret = x.clone()
        x = self.head(x)                                        # [B, T, J, dim_out]
        if self.desired_return == "final":
            ret = x.clone()
            
        return ret

    def get_representation(self, x):
        assert self.desired_return == "representation", "desired_return must be 'representation' to use get_representation"
